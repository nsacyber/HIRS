...
if (!$skippki) {
	...
	pwsh -ExecutionPolicy Bypass $global:HIRS_REL_SCRIPTS_PKI_SETUP -LOG_FILE:"$global:LOG_FILE" -PKI_PASS:"$HIRS_PKI_PWD" -UNATTENDED:"$unattended"
...
if (!$skipdb) {
	pwsh -ExecutionPolicy Bypass $global:HIRS_REL_SCRIPTS_DB_CREATE -LOG_FILE:"$global:LOG_FILE" -UNATTENDED:"$unattended"
    if ($LastExitCode -eq 0) { 
