...
Function set_hirs_db_pwd () {
	...
	if (!$DB_ADMIN_PWD) {
		Write-Output "set_hirs_db_pwd was called without supplying a required variable" | WriteAndLog
	}
	...
	if ($Env:HIRS_DB_PWD) {
		...
		Write-Output "Using hirs_db password found in the ACA properties file $global:HIRS_DATA_ACA_PROPERTIES_FILE" | WriteAndLog
	} else {
		Write-Output "Using randomly generated password for the DB key password" | WriteAndLog
    ...
    if ($RESULT -eq 1) {
        Write-Output "hirs-db user exists" | WriteAndLog
    }
...
Function create_hirs_db_with_tls () {
	...
	if (!$DB_ADMIN_PWD) {
		Write-Output "create_hirs_db_with_tls: DB_ADMIN_PWD not provided and is required" | WriteAndLog
	}
	if (!$HIRS_PASS) {
		Write-Output "create_hirs_db_with_tls: HIRS_PASS not provided and is required" | WriteAndLog
	}
