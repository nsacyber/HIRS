...
As the computer boots, UEFI has each stage of boot software measure the next stage before it loads.
...
1. Extended into a TPM Platform Configuration Register (PCR) slot, per the image below
    ...
    - Most TPMs have 24 PCRs per supported hash algorithm. TPM 1.2 supports SHA-1 (24 PCRs). 
      TPM 2.0 supports SHA-1 and SHA-256 at the minimum (48 PCRs minimum).
...
The image below shows the interaction of Secure Boot and TPM with UEFI boot phases:

<img src= "../../../images/bg-op-uefi-pcrs.png" alt="UEFI Captures Firmware Info PCRs Pic">
...
Applications and drivers can also extend runtime hashes to dedicated TPM PCRs, but the original 
firmware Event Log does not include these either as they occur at a later time.