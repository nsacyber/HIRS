...
public class X509AttributeCredential extends PlatformCredential {
    ...
    private static final Logger LOGGER = LogManager.getLogger(X509AttributeCredential.class);
    ...
    public boolean isSignatureValid(final ContentVerifierProvider verifierProvider)
        ...
        return verifier.verify(DERBitString.getInstance(getBaseEncoded(SIGNATURE)).getOctets());
    }
    ...
    public int getX509CredentialVersion() {
        try {
            AttributeCertificatePkc attriPkc = new AttributeCertificatePkc(getRawBytes());
    ...
    private ASN1Object getBaseEncoded(final int index) {
        ...
        try {
            ...
            switch (index) {
                case SIGNATURE:
                    return DERBitString.getInstance(topLevelItems.getObjectAt(SIGNATURE));
                case CONTENT_BODY:
                case ALGORITHM:
                default:
                    return null;
